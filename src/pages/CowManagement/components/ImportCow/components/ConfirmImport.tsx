  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      if (invalidHealthRecords.length > 0) {
        toast.error(
          t('Một số hồ sơ sức khỏe có thông tin không hợp lệ (status)!')
        );
        return;
      }
      ...
      const healthRecords = reviewData.map((cow) => ({
        ...
        size: cow.healthInfoResponses[0]?.health?.size || 0,
        bodyTemperature:
          cow.healthInfoResponses[0]?.health?.bodyTemperature || 0,
        heartRate: cow.healthInfoResponses[0]?.health?.heartRate || 0,
        respiratoryRate:
          cow.healthInfoResponses[0]?.health?.respiratoryRate || 0,
        ruminateActivity:
          cow.healthInfoResponses[0]?.health?.ruminateActivity || 0,
        chestCircumference:
          cow.healthInfoResponses[0]?.health?.chestCircumference || 0,
        bodyLength: cow.healthInfoResponses[0]?.health?.bodyLength || 0,
        description:
          cow.healthInfoResponses[0]?.health?.description || 'No description',
      }));
      ...
      const healthRecordErrors =
        response?.data?.healthRecordsResponse?.errors || [];
      ...
      if (
        ...
        error.response?.data?.message === 'Invalid import times'
      ) {
