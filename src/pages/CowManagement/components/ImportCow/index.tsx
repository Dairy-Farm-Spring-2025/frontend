...
const ListCowImport = () => {
  ...
  const availableCows = useMemo((): Cow[] => {
    return importedCowIds.map((id, index) => {
      ...
      const cowType = dataCowType?.find(
        (type) =>
          type.name === (reviewCow.cowTypeName || reviewCow.cowType?.name)
      ) || {
        cowTypeId: 0,
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      filterable: true,
      filterOptions:
        dataCowType?.map((type) => ({ text: type.name, value: type.name })) ||
        [],
    },
    {
      dataIndex: 'cowStatus',
      ...
      prev.map((item) =>
        item.key === key
          ? {
              ...
              cowType:
                field === 'cowTypeName'
                  ? { ...item.cowType, name: value }
                  : item.cowType,
              cowTypeEntity:
                field === 'cowTypeName'
                  ? { ...item.cowTypeEntity, name: value }
                  : item.cowTypeEntity,
            }
          : item
      )
                  ...
                  <div className="flex flex-col gap-2 text-base bg-white">
                    {!validateCows && (
                      <span className="text-red-500">
                        -{' '}
                        {t(
                          'Some cows have invalid or missing fields (name, status, gender, origin, type)'
                        )}
                      </span>
                    )}
                    {!validateHealthRecords && (
                      <span className="text-red-500">
                        -{' '}
                        {t(
                          'Some health records have invalid or missing statuses'
                        )}
                      </span>
                    )}
                    {!hasReviewErrors && (
