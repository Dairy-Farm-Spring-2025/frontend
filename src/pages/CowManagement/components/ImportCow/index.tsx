...
const ListCowImport = () => {
  ...
  console.log(importedCowIds, importSuccess);
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      const response = await importTrigger({
      ...
      if (response?.data?.successes?.length > 0) {
        message.success(
          `Đã nhập thành công ${response.data.successes.length} con bò!`
        );
        setImportedCowIds(
          response.data.successes.map((cow: { cowId: number }) => cow.cowId)
        );
      ...
      if (response?.data?.errors?.length > 0) {
        ...
        message.warning(
          `Có ${response.data.errors.length} lỗi xảy ra. Kiểm tra console để biết thêm chi tiết.`
        );
      }
    } catch (error: any) {
