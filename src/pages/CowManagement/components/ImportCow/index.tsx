  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      if (invalidHealthRecords.length > 0) {
        ...
        message.error(
          'Một số hồ sơ sức khỏe có thông tin không hợp lệ (status)!'
        );
        return;
      }
      ...
      const healthRecords = reviewData.map((cow: any) => ({
        ...
        chestCircumference: Math.round(
          cow.healthRecord?.chestCircumference || 0
        ),
      ...
      console.log(
        'Payload being sent to CREATE_BULK API:',
      ...
      if (response?.data?.cowsResponse?.successes?.length > 0) {
        ...
        setImportedCowIds(
          response.data.cowsResponse.successes.map(
