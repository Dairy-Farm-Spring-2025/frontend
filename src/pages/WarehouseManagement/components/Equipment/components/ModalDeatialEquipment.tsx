...
const ModalDetailEquipment = ({
    ...
    const { data: dataWarehouse } = useFetcher<WarehouseType[]>('warehouses', 'GET');
    ...
    useEffect(() => {
        if (data) {
            form.setFieldsValue({
                ...
                warehouseLocationEntity: data.warehouseLocationEntity?.warehouseLocationId,
            });
        }
    ...
    const items: DescriptionPropsItem['items'] = [

        {
            ...
            children: !edit ? (
                ...
                <FormItemComponent name="warehouseLocationEntity" rules={[{ required: true }]}>
                    <SelectComponent
                        options={dataWarehouse?.map((warehouse) => ({
